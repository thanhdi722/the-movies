...
export default function AnimePage() {
    ...
    console.log("moviesDataDetail", moviesData)
    ...
    return (
        <main className="min-h-screen bg-[#121212] p-4 md:p-6">
            <div className="max-w-7xl mx-auto">
                {selectedEpisode ? (
                    <div className="space-y-4">
                        ...
                        <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">
                            {movie?.name} - {episodes.find((ep: any) => ep.link_embed === selectedEpisode)?.name}
                        </h2>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4 relative">
                            <div className="relative w-full h-0 pb-[50%]">
                                <Image
                                    src={movie?.thumb_url ? movie.thumb_url : '/path/to/fallback-image.jpg'}
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="flex gap-2 text-gray-300">
                                    ...
                                    <div className="space-x-2">
                                        {episodes.slice(-3).map((ep: any) => (
                                            <button key={ep.slug} className="bg-[#ff7a10] text-white px-2 py-1 rounded-md">
                                                {ep.name}
                                            </button>
                                        ))}
                                    </div>
                            ...
                            <div className="flex gap-2">
                                {movie?.category.map((cat: any) => (
                                    <button key={cat.id} className="bg-[#ff7a10] text-white px-2 py-1 rounded-md">{cat.name}</button>
                                ))}
                            </div>
                        </div>
                    </div>
                ...
                {/* Episode List */}
                <div className="mt-8">
                    <h2 className="text-xl font-bold text-yellow-500 mb-4 bg-gradient-to-r from-[#ff8a00] to-[#ff2070] bg-clip-text text-transparent box-decoration-clone bg-[#ff7a10]">Tập phim</h2>
                    <div className="grid grid-cols-6 md:grid-cols-12 gap-2">
                        {episodes.map((ep: any) => (
                            <button
                                key={ep.slug}
                                onClick={() => handleEpisodeClick(ep)}
                                className={`px-2 py-1 rounded-md ${selectedEpisode === ep.link_embed ? 'bg-[#ff7a10]' : 'bg-gray-500'
                                    } text-white`}
                            >
                                {ep.name}
                            </button>
